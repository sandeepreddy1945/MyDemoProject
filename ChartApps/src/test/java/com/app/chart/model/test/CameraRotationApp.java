...
public class CameraRotationApp extends Application {
	...
	private Parent createContent() throws Exception {
		...
		Timeline timeline = new Timeline(new KeyFrame(Duration.seconds(0), new KeyValue(yRotate.angleProperty(), 0)),
				new KeyFrame(Duration.seconds(15), new KeyValue(yRotate.angleProperty(), 360)));
		...
		// set the pivot for the camera position animation base upon mouse clicks on
		// objects
		root.getChildren().stream().filter(node -> !(node instanceof Camera))
